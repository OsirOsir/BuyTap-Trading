...
Author: Philip Osir
*/
// Complete System Pending, Active, Closed and revoked //
...
function buytap_pair_orders($order_id) {
    ...
    // Case 1: Buyer is being paired (order is Pending)
    if ($order_status === 'Pending') {
        // Look for unpaired matured seller
        $sellers = get_posts([
            'post_type' => 'buytap_order',
            'posts_per_page' => 1,
            'meta_query' => [
        ...
        if (!empty($sellers)) {
            $seller = $sellers[0];
            $seller_id = $seller->ID;
            buytap_perform_pairing($order_id, $seller_id, 'buyer');
        }
    }
    // Case 2: Seller is being paired (order is Matured)
    elseif ($order_status === 'Matured') {
        $is_paired = get_post_meta($order_id, 'is_paired', true);
        if ($is_paired !== 'yes') {
            // Look for oldest pending buyer
            $buyers = get_posts([
                'post_type' => 'buytap_order',
                'posts_per_page' => 1,
                'meta_query' => [
            ...
            if (!empty($buyers)) {
                ...
                buytap_perform_pairing($buyer_id, $order_id, 'seller');
            }
        }
    }
}
...
/**
 * Performs the actual pairing between two orders
 */
function buytap_perform_pairing($buyer_order_id, $seller_order_id, $initiator) {
    ...
    // Get buyer info
    $buyer_user = get_userdata(get_post_field('post_author', $buyer_order_id));
    $buyer_name = $buyer_user ? $buyer_user->display_name : '';
    $buyer_number = get_user_meta($buyer_user->ID, 'mobile_number', true);
    ...
    error_log("✅ Successfully paired Buyer $buyer_order_id with Seller $seller_order_id (initiated by $initiator)");
}
