...
add_shortcode('buytap_pending_orders', function () {
    ...
    echo '<div class="buytap-orders-table">'; // ✅ Universal wrapper starts here
    ...
    if ($orders) {
        ...
        foreach ($orders as $order) {
            ...
            echo '<tr style="border-bottom:1px solid #ddd;">
                    <td>' . esc_html(get_post_meta($id, 'order_date', true)) . '</td>
                    <td>' . esc_html(get_post_meta($id, 'order_details', true)) . '</td>
                    <td>' . esc_html(get_post_meta($id, 'status', true)) . '</td>
                    <td><strong>Ksh</strong> ' . number_format(get_post_meta($id, 'amount_to_make', true)) . '</td>
                </tr>';
        }
