...
// 		  let storageListenerAttached = false;  // ✅ add this line here
        ...
        document.addEventListener('DOMContentLoaded', function () {
		  ...
		  const LS_KEY = `buyFormHideAt_${selectorKey}_${pageKey}${keySuffix ? '_' + keySuffix : ''}`;
			...
			// 🔁 Global keys (shared across tabs)
				const G_KEY  = 'buytapSale_hideAt';
				const G_STAT = 'buytapSale_status';
				const G_NEXT = 'buytapSale_nextOpenMs';   // <— NEW: global next-open time (UTC ms)
				const G_CFG  = 'buytapSale_cfgHash';      // <— NEW: config signature

				// Quick, stable hash for config
				function hash32(s){
				  let h = 0;
				...
				// Build a config signature so all tabs agree on the same schedule
				const cfgString = JSON.stringify({
				  tz, mode, testWait, showFor,
				  openTimes: openTimes.map(t => t.trim()),
				  openDates
				});
				const cfgHash = hash32(cfgString);

				// Cross-tab sync listener (next-open + open-window)
				if (!window.buytapStorageListenerAttached) {
				  window.addEventListener('storage', function (e) {
					...
					if (e.key === G_NEXT || e.key === G_CFG) {
					  if (localStorage.getItem(G_CFG) === cfgHash) {
						const n = localStorage.getItem(G_NEXT);
						if (n) {
						  targetTime = DateTime.fromMillis(parseInt(n, 10), { zone: 'utc' }).setZone(tz);
						  requestAnimationFrame(updateCountdown);
						}
					  }
					}
          ...
          function getNextDropTimeLocal() {
			  ...
			  return nowInZone().plus({ seconds: testWait });
							...
							// Publish/consume a single global next-open target for all tabs
				function loadOrPublishNextTarget() {
				  const storedCfg  = localStorage.getItem(G_CFG);
				  const storedNext = localStorage.getItem(G_NEXT);

				  if (storedCfg === cfgHash && storedNext) {
					const candidate = DateTime.fromMillis(parseInt(storedNext, 10), { zone: 'utc' }).setZone(tz);
					if (candidate > nowInZone()) return candidate; // still future
				  }

				  // Compute locally and publish for everyone
				  const localTarget = getNextDropTimeLocal();
				  localStorage.setItem(G_CFG,  cfgHash);
				  localStorage.setItem(G_NEXT, String(localTarget.toUTC().toMillis()));
				  return localTarget;
				}
          ...
          function updateCountdown() {
            const now = nowInZone();
			  // If another tab recomputed the target or config changed, adopt it
			  if (localStorage.getItem(G_CFG) === cfgHash) {
				  const globNext = localStorage.getItem(G_NEXT);
				  if (globNext) {
					  const candidate = DateTime.fromMillis(parseInt(globNext, 10), { zone: 'utc' }).setZone(tz);
          ...
          // Start — set initial visibility based on persisted state to avoid flicker
			...
			(function adoptInitialGlobalTarget(){
			  if (localStorage.getItem(G_CFG) === cfgHash) {
				const n = localStorage.getItem(G_NEXT);
				if (n) targetTime = DateTime.fromMillis(parseInt(n,10), { zone:'utc' }).setZone(tz);
			  }
			})();
			requestAnimationFrame(updateCountdown);
        });
