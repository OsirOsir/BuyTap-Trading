...
function buytap_admin_cm_render_page() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buytap_admin_cm_nonce']) && wp_verify_nonce($_POST['buytap_admin_cm_nonce'], 'buytap_admin_cm_create')) {
        $user_id = intval($_POST['user_id'] ?? 0);
        $amount = floatval($_POST['amount'] ?? 0);
        $notes = sanitize_text_field($_POST['notes'] ?? '');
        $duration = intval($_POST['duration'] ?? 0);

        // Set profit percentage based on duration
        $profit_percentage = match ($duration) {
            4 => 0.30,
            8 => 0.65,
            12 => 0.95,
            default => 0,
        ...
        $expected_return = $amount + ($amount * $profit_percentage);
        ...
        if ($user_id <= 0 || $amount <= 0) {
            ...
            if (is_wp_error($order_id) || !$order_id) {
                ...
                update_post_meta($order_id, 'order_details', sprintf('%s for %d%% in %d Days. %s',
                    number_format($amount, 0), $profit_percentage * 100, $duration, $notes
                ));

                update_post_meta($order_id, 'amount_bought', $amount);
                update_post_meta($order_id, 'expected_return', $expected_return);
                update_post_meta($order_id, 'amount_to_make', $amount);
                update_post_meta($order_id, 'remaining_to_receive', $amount);
                update_post_meta($order_id, 'is_paired', 'no');

                // Auto-pair if available
                if (buytap_admin_cm_pairer_available()) {
