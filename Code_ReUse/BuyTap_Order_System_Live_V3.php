...
function buytap_process_order_submission($amount, $duration, $buyer_id = 0) {
	...
	// 🔒 Enforce time window
    if ( ! buytap_is_form_open_now() ) {
        buytap_redirect_with_error("Purchase window has closed. Please wait for the next opening."); 
    }
...
/**
 * Checks whether the BuyTap purchase form is currently within an open window.
 *
 * Modes:
 * - test:   Opens once after "buytap_test_start_time" and stays open for X seconds
 *           (controlled by buytap_test_wait_seconds + buytap_show_for_seconds).
 * - schedule: Uses configured daily open times (buytap_open_times) in the given
 *             timezone (buytap_timezone), and keeps the form open for the duration
 *             of buytap_show_for_seconds after each opening.
 *
 * @return bool True if the purchase form should be open right now, false otherwise.
 */
function buytap_is_form_open_now() {
    $mode = get_option('buytap_mode', 'test');
    $show_for = (int) get_option('buytap_show_for_seconds', 3600);

    if ($mode === 'test') {
        // For test mode: opened X seconds after first visit, then closes
        $start = (int) get_option('buytap_test_start_time', 0);
        if (!$start) {
            // First time: set start
            $start = time();
            update_option('buytap_test_start_time', $start);
        }
        return (time() >= $start && time() <= $start + $show_for);
    }

    if ($mode === 'schedule') {
        // For schedule mode: check against daily times
        $timezone = new DateTimeZone(get_option('buytap_timezone','Africa/Nairobi'));
        $now = new DateTime('now', $timezone);

        $open_times = explode(',', get_option('buytap_open_times','09:00,19:00'));
        foreach ($open_times as $open) {
            $open_dt = DateTime::createFromFormat('H:i', trim($open), $timezone);
            if (!$open_dt) continue;

            $open_ts = $open_dt->getTimestamp();
            $close_ts = $open_ts + $show_for;
...
add_action('rest_api_init', function () {
    register_rest_route('buytap/v1', '/form-open/', [
        'methods'  => 'GET',
        'callback' => function () {
            return [ 'open' => buytap_is_form_open_now() ];
        },
...
function buytap_enqueue_script_and_nonce() {
    ...
    wp_localize_script('buytap-front-js', 'buytapData', array(
        ...
        'rest_form_open'          => esc_url_raw(rest_url('buytap/v1/form-open')), // ✅ NEW
    ));
}
