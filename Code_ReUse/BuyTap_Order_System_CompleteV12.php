...
add_shortcode('buytap_purchase_form', function () {
    if (!is_user_logged_in()) {
        return '<p class="elementor-message elementor-message-danger">⚠️ Please log in to purchase tokens.</p>';
    }
	$user_error = get_transient('buytap_error_' . get_current_user_id());
	if ($user_error) {
		?>
		<div class="buytap-error" style="margin:10px 0;padding:12px;background:rgba(255,0,0,0.1);border:1px solid #e00;color:#e00;border-radius:6px;">
			<?= esc_html($user_error); ?>
		</div>
		<?php
    ...
    ob_start(); ?>
    <form id="buy-form" class="elementor-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        <div class="elementor-form-fields-wrapper">
            ...
            <div class="elementor-field-type-number elementor-field-group elementor-column elementor-col-100">
                ...
                <input type="number" id="buytap_amount" name="buytap_amount"
                       ...
                       placeholder="Enter token amount" required min="500" max="5000">
            </div>
            ...
            <div class="elementor-field-type-select elementor-field-group elementor-column elementor-col-100">
                ...
                <select id="buytap_duration" name="buytap_duration"
                    ...
                    <option value="4">30% in 4 Days</option>
                    <option value="8">65% in 8 Days</option>
                    <option value="12">95% in 12 Days</option>
                </select>
            </div>
...
function buytap_process_order_submission($amount, $duration, $buyer_id = 0) {
	...
	if ($amount > $available) {
		$msg = $available <= 0
			? "❌ Purchase unavailable — token pool is empty. Please try again later."
			: "Only {$available} tokens are available. Please lower your amount.";
		buytap_redirect_with_error($msg);
	}

    // 🔢 Duration logic — accept numeric or label values
    $duration_days  = 0;
    $profit_percent = 0;
    switch (trim($duration)) {
        case '4':
        case '4 Days':
        case '30% in 4 Days':
            $duration_days  = 4;
