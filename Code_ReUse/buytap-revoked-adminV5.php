...
add_action('admin_post_buytap_reinstate_order', function () {
    ...
    // ✅ Preserve successful chunks first!
    buytap_preserve_successful_chunks($order_id);
    
    // ✅ Get only the revoked amount to deduct from pool
    $revoked_amount = buytap_get_revoked_amount($order_id);
    
    if ($revoked_amount > 0) {
        $pool = (int) get_option('buytap_total_tokens', 0);
        update_option('buytap_total_tokens', max(0, $pool - $revoked_amount));
    }
    
    // ✅ Calculate remaining balance for pairing
    $total_amount = (int) get_post_meta($order_id, 'amount_to_send', true);
    $confirmed_amount = buytap_get_confirmed_amount($order_id);
    $remaining_to_pair = max(0, $total_amount - $confirmed_amount);
    // ✅ KEEP THESE IMPORTANT UPDATES:
    update_post_meta($order_id, 'status', 'Pending');
    update_post_meta($order_id, 'sub_status', 'Pending');
    update_post_meta($order_id, 'remaining_to_send', $remaining_to_pair);
    update_post_meta($order_id, 'is_paired', $remaining_to_pair > 0 ? 'yes' : 'no');
    
    // ✅ KEEP THESE CLEANUP OPERATIONS:
    ...
    // ✅ Pair only the remaining amount
    if ($remaining_to_pair > 0 && function_exists('buytap_pair_orders')) {
...
/**
 * Get the amount that was revoked (from the problematic seller)
 */
function buytap_get_revoked_amount($buyer_id) {
    global $wpdb;
...
/**
 * Get the amount that was successfully confirmed by sellers
 */
function buytap_get_confirmed_amount($buyer_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'buytap_chunks';
    ...
    return (float) $wpdb->get_var($wpdb->prepare(
        "SELECT COALESCE(SUM(amount), 0) 
         FROM $table_name 
         WHERE buyer_order_id = %d 
         AND status IN ('Received', 'Payment Made')",
        $buyer_id
    ));
}

/**
 * Preserve successful chunks when reinstating - reset only revoked chunks
 */
function buytap_preserve_successful_chunks($buyer_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'buytap_chunks';
    
    // Only reset the revoked chunks back to 'Awaiting Payment'
    ...
    // Keep successful chunks exactly as they are
    // ('Received' and 'Payment Made' remain unchanged)
}
