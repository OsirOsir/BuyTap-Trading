...
add_action('admin_post_buytap_force_revoke_order', function () {
    ...
    // Mark buyer order as Revoked
    update_post_meta($order_id, 'status', 'Revoked');
    update_post_meta($order_id, 'sub_status', 'Payment Timeout');
    update_post_meta($order_id, 'date_revoked', current_time('mysql'));
    update_post_meta($order_id, 'revoked_reason', 'Admin forced revoke.');
    update_post_meta($order_id, 'is_paired', 'no');
