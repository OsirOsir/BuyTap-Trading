...
add_shortcode('buytap_purchase_form', function () {
    ...
    $login_url    = 'http://localhost/buytap/login/'; // redirect back here after login

    // Show error only for logged-in users
    if ($is_logged_in) {
        $user_error = get_transient('buytap_error_' . get_current_user_id());
        if ($user_error) {
            echo '<div class="buytap-error">' . esc_html($user_error) . '</div>';
        ...
        <div class="elementor-form-fields-wrapper">
            ...
            <div class="elementor-field-type-select elementor-field-group elementor-column elementor-col-100">
                ...
                <select id="buytap_duration" name="buytap_duration"
                    ...
                    <option value="4" <?php selected($prefill_duration, '4'); ?>>30% in 4 Days</option>
                    <option value="8" <?php selected($prefill_duration, '8'); ?>>65% in 8 Days</option>
                    <option value="12" <?php selected($prefill_duration, '12'); ?>>95% in 12 Days</option>
                </select>
            </div>
...
function buytap_handle_purchase() {
    ...
    $login_url = 'http://localhost/buytap/login/';
...
function buytap_process_order_submission($amount, $duration, $buyer_id = 0) {
    ...
    $msg = "❌ Purchase unavailable — not enough tokens left in the pool. Please try again later.";
