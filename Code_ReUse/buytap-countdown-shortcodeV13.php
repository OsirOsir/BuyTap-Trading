...
(function () {
  ...
  function buytapBoot() {
    ...
    // AJAX function to check server time and close form if needed
    async function checkServerTimeAndCloseIfNeeded() {
      try {
        if (!openUntilMs) return; // nothing to do
        
        const response = await fetch('<?php echo rest_url("buytap/v1/now"); ?>', {
          cache: 'no-store'
        });
        ...
        const serverUtcNowMs = data.utc * 1000; // Convert to milliseconds
        
        // If server time has passed the openUntilMs, close the form immediately
        ...
        // If AJAX fails, fall back to client-side timing
      }
    }
    ...
    function showFormThenHideLater() {
      ...
      if (!closeSyncTimer) {
        closeSyncTimer = setInterval(checkServerTimeAndCloseIfNeeded, 3000);
      }
      
      // Only bind focus/visibility once globally
      if (!window._btCloseSyncBound) {
        window.addEventListener('focus', checkServerTimeAndCloseIfNeeded);
        document.addEventListener('visibilitychange', () => {
          if (document.visibilityState === 'visible') checkServerTimeAndCloseIfNeeded();
        });
    ...
    if (Number.isFinite(storedHideAt) && storedStatusInit === "open" && nowStartUTCms < storedHideAt) {
      ...
      // Start server sync if form is already open
      if (!closeSyncTimer) {
        closeSyncTimer = setInterval(checkServerTimeAndCloseIfNeeded, 3000);
      }
    } else {
