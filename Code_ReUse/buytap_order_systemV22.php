...
function buytap_pair_orders($order_id) {
	...
	// ✅ Track sellers already paired in this run
    $paired_sellers = [];

    foreach ($matches as $match) {
        $match_id = $match->ID;
		
		// Skip if this seller was already paired to this buyer in this run
        if ($is_buyer) {
            if (in_array($match_id, $paired_sellers, true)) {
                continue;
            }
        } else {
            if (in_array($order_id, $paired_sellers, true)) {
                continue;
            }
        }
        ...
        if ($result) {
			// ✅ Record the pairing so it won't repeat in this loop
            if ($is_buyer) {
                $paired_sellers[] = $match_id;
            } else {
                $paired_sellers[] = $order_id;
            }
    ...
    // Update status to 'paired' only if completely matched
	if ($status === 'Pending' && buytap_buyer_remaining($order_id) <= 0) {
...
function buytap_perform_pairing($buyer_order_id, $seller_order_id, $initiator) {
	...
	// Recalculate latest remaining amounts to avoid over-pairing
    ...
    if ($buyer_remaining <= 0 || $seller_remaining <= 0) {
        return false; // Already fully matched
    }
    ...
    // Mark buyer as paired only if fully matched (no remaining_to_send)
