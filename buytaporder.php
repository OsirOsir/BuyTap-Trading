...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // }
        function getNextDropTime() {
          const now = DateTime.now().setZone("Africa/Nairobi");
          return now.plus({ minutes: 1 }); // ⏱ For testing: opens in 60 seconds
        }
...
window.addEventListener("storage", function (event) {
  if (event.key === "buyFormHideAt") {
    const hideAt = event.newValue;
    const countdownWrapper = document.getElementById("custom-countdown");

    if (hideAt) {
      const now = DateTime.now().setZone("Africa/Nairobi");
      const hideAtTime = DateTime.fromISO(hideAt);
      if (now < hideAtTime) {
        if (formEl) formEl.style.display = "block";
        if (countdownWrapper) countdownWrapper.style.display = "none";
      ...
      // If hideAt is removed, restart timer
      if (formEl) formEl.style.display = "none";
      if (countdownWrapper) countdownWrapper.style.display = "block";
      targetTime = getNextDropTime();
      updateCountdown();
...
//   const now = DateTime.now().setZone("Africa/Nairobi");
//   const hideAt = now.plus({ minutes: 1 }); // Show form for 1 minute
//   localStorage.setItem("buyFormHideAt", hideAt.toISO());
...
//     targetTime = getNextDropTime(); // Start next cycle
//     updateCountdown();
//   }, 60000); // 1 minute
